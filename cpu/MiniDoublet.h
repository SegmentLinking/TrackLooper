    ...
    namespace CPU
    {
        class MiniDoublet
        {
            ...
            public:
                ...
                void runMiniDoubletAlgo(MDAlgo algo, SDL::CPU::LogLevel logLevel=SDL::CPU::Log_Nothing);

                // The following algorithm does nothing and accepts the mini-doublet
                void runMiniDoubletAllCombAlgo();

                // The default algorithms;
                void runMiniDoubletDefaultAlgo(SDL::CPU::LogLevel logLevel);
                void runMiniDoubletDefaultAlgoBarrel(SDL::CPU::LogLevel logLevel);
                void runMiniDoubletDefaultAlgoEndcap(SDL::CPU::LogLevel logLevel);
                ...
                static float fabsdPhiPixelShift(const Hit& lowerHit, const Hit& upperHit, const Module& lowerModule, SDL::CPU::LogLevel logLevel=SDL::CPU::Log_Nothing);

                // The math for shifting the strip hit up or down along the PS module orientation (deprecated)
                static float fabsdPhiStripShift(const Hit& lowerHit, const Hit& upperHit, const Module& lowerModule, SDL::CPU::LogLevel logLevel=SDL::CPU::Log_Nothing);

                // The math for shifting the strip hit up or down along the PS module orientation, returns new x, y and z position
                static std::tuple<float, float, float> shiftStripHits(const Hit& lowerHit, const Hit& upperHit, const Module& lowerModule, SDL::CPU::LogLevel logLevel=SDL::CPU::Log_Nothing);

                // The function to actually determine whether a pair of hits is a reco-ed mini doublet or not
                static bool isHitPairAMiniDoublet(const Hit& lowerHit, const Hit& upperHit, const Module& lowerModule, MDAlgo algo, SDL::CPU::LogLevel logLevel=SDL::CPU::Log_Nothing);
