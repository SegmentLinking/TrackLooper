...
__device__ float SDL::computeRadiusUsingRegression(int nPoints, float* xs, float* ys, float& g, float& f, float* sigmas, float& chiSquared)
{
    ...
    for(size_t i = 0; i < nPoints; i++)
    {
        ...
        float sigma = 2 * sqrtf(xs[i] * xs[i] + ys[i] * ys[i]) * sigmas[i];
