...
cudaStreamSynchronize(stream);
        ...
        tripletsInCPU->betaIn  = new __half[nMemoryLocations];
        tripletsInCPU->betaOut = new __half[nMemoryLocations];
        tripletsInCPU->pt_beta = new __half[nMemoryLocations];
        ...
        cudaMemcpyAsync(tripletsInCPU->betaIn,   tripletsInGPU->betaIn, nMemoryLocations * sizeof(__half), cudaMemcpyDeviceToHost,stream);
        cudaMemcpyAsync(tripletsInCPU->betaOut, tripletsInGPU->betaOut, nMemoryLocations * sizeof(__half), cudaMemcpyDeviceToHost,stream);
        cudaMemcpyAsync(tripletsInCPU->pt_beta, tripletsInGPU->pt_beta, nMemoryLocations * sizeof(__half), cudaMemcpyDeviceToHost,stream);
...
cudaStreamSynchronize(stream);
        ...
        quintupletsInCPU->innerRadius = new __half[nMemoryLocations];
        quintupletsInCPU->outerRadius = new __half[nMemoryLocations];
        ...
        cudaMemcpyAsync(quintupletsInCPU->innerRadius, quintupletsInGPU->innerRadius, nMemoryLocations * sizeof(__half), cudaMemcpyDeviceToHost,stream);
        cudaMemcpyAsync(quintupletsInCPU->outerRadius, quintupletsInGPU->outerRadius, nMemoryLocations * sizeof(__half), cudaMemcpyDeviceToHost,stream);
...
cudaStreamSynchronize(stream);
        ...
        pixelTripletsInCPU->pixelRadius = new __half[nPixelTriplets];
        pixelTripletsInCPU->pixelRadiusError = new __half[nPixelTriplets];
        pixelTripletsInCPU->tripletRadius = new __half[nPixelTriplets];
        pixelTripletsInCPU->isDup = new bool[nPixelTriplets];
        pixelTripletsInCPU->eta = new __half[nPixelTriplets];
        pixelTripletsInCPU->phi = new __half[nPixelTriplets];
        pixelTripletsInCPU->score = new __half[nPixelTriplets];
        ...
        cudaMemcpyAsync(pixelTripletsInCPU->pixelRadius, pixelTripletsInGPU->pixelRadius, nPixelTriplets * sizeof(__half), cudaMemcpyDeviceToHost,stream);
        cudaMemcpyAsync(pixelTripletsInCPU->tripletRadius, pixelTripletsInGPU->tripletRadius, nPixelTriplets * sizeof(__half), cudaMemcpyDeviceToHost,stream);
        ...
        cudaMemcpyAsync(pixelTripletsInCPU->eta, pixelTripletsInGPU->eta, nPixelTriplets * sizeof(__half), cudaMemcpyDeviceToHost,stream);
        cudaMemcpyAsync(pixelTripletsInCPU->phi, pixelTripletsInGPU->phi, nPixelTriplets * sizeof(__half), cudaMemcpyDeviceToHost,stream);
        cudaMemcpyAsync(pixelTripletsInCPU->score, pixelTripletsInGPU->score, nPixelTriplets * sizeof(__half), cudaMemcpyDeviceToHost,stream);
cudaStreamSynchronize(stream);
    }
...
cudaStreamSynchronize(stream);
        ...
        pixelQuintupletsInCPU->score = new __half[nPixelQuintuplets];
        ...
        cudaMemcpyAsync(pixelQuintupletsInCPU->score, pixelQuintupletsInGPU->score, nPixelQuintuplets * sizeof(__half), cudaMemcpyDeviceToHost,stream);
cudaStreamSynchronize(stream);
    }
