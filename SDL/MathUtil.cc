...
float SDL::CPU::MathUtil::ATan2(float y, float x)
{
...
float SDL::CPU::MathUtil::ptEstimateFromDeltaPhiChangeAndRt(float dPhiChange, float rt)
{
    return rt * 2.99792458e-3 * 3.8 / 2. / std::sin(dPhiChange);
}

float SDL::CPU::MathUtil::ptEstimateFromRadius(float radius)
{
    return 2.99792458e-3 * 3.8 * radius;
}

float SDL::CPU::MathUtil::dphiEstimateFromPtAndRt(float pt, float rt)
{
    return std::asin(rt / (2 * pt / (2.99792458e-3 * 3.8)));
}

SDL::CPU::Hit SDL::CPU::MathUtil::getCenterFromThreePoints(SDL::CPU::Hit& hitA, SDL::CPU::Hit& hitB, SDL::CPU::Hit& hitC)
{
    ...
    if ((slope_AB - slope_BC) == 0)
    {
        ...
        std::cout << "SDL::CPU::MathUtil::getCenterFromThreePoints() function the three points are in straight line!" << std::endl;
        return SDL::CPU::Hit();
    }
    ...
    return SDL::CPU::Hit(x, y, 0);

}

float SDL::CPU::MathUtil::angleCorr(float dr, float pt, float angle)
{
