...
CXXFLAGS    = $(ROOTCFLAGS) -ISDL -I$(shell pwd) -Icode -Icode/core
CFLAGS      = $(ROOTCFLAGS) --compiler-options -Wall --compiler-options -Wno-unused-function --compiler-options -g --compiler-options -O2 --compiler-options -fPIC --compiler-options -fno-var-tracking -ISDL -I$(shell pwd) -Icode -Icode/core --compiler-options -fopenmp
EXTRACFLAGS = $(shell rooutil-config)
EXTRAFLAGS  = -fPIC -ITMultiDrawTreePlayer -Wunused-variable -lTMVA -lEG -lGenVector -lXMLIO -lMLP -lTreePlayer -lcudart -fopenmp -L${CUDA_HOME}/lib64
DOQUINTUPLET = -DFP16_Base -DFP16_dPhi
PRELOAD     =
PT0P8       =
CMSSW12GEOM =
T3T3EXTENSION=
...
bin/doAnalysis: bin/doAnalysis.o $(OBJECTS)
	$(LD) $(PT0P8) $(CMSSW12GEOM) $(PRELOAD) $(T3T3EXTENSION) $(LDFLAGS) $^ $(ROOTLIBS) $(EXTRACFLAGS) $(CUTVALUEFLAG) $(PRIMITIVEFLAG) $(EXTRAFLAGS) $(DOQUINTUPLET) -o $@

bin/sdl: bin/sdl.o $(OBJECTS)
	$(LD) $(PT0P8) $(CMSSW12GEOM) $(PRELOAD) $(T3T3EXTENSION) $(LDFLAGS) $^ $(ROOTLIBS) $(EXTRACFLAGS) $(CUTVALUEFLAG) $(PRIMITIVEFLAG) $(EXTRAFLAGS) $(DOQUINTUPLET) -o $@

%.o: %.cc
	$(CC) $(PT0P8) $(CMSSW12GEOM) $(PRELOAD) $(T3T3EXTENSION) $(CFLAGS) $(EXTRACFLAGS) $(CUTVALUEFLAG) $(PRIMITIVEFLAG) $(DOQUINTUPLET) $< -dc -o $@
