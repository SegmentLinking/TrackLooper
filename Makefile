...
CC          = nvcc
CXX         = nvcc
CXXFLAGS    = -g -O2 --compiler-options -Wall --compiler-options -fPIC --compiler-options -Wshadow --compiler-options -Woverloaded-virtual -G -lineinfo  -fopenmp -lgomp #-fsanitize=thread #--default-stream-per-thread
LD          = g++
