...
# What i think are OK connections
def isOKConnBarrelEndcap(key1, key2):
    if key1 == (1, 5) and (key2 == (1, 4)): return True
    if key1 == (2, 5) and (key2 == (1, 4)): return True
    if key1 == (3, 5) and (key2 == (1, 4)): return True
    if key1 == (4, 5) and (key2 == (1, 4)): return True
    if key1 == (5, 5) and (key2 == (1, 4)): return True
    return False

# What i think are OK connections
def isOKConnEndcapEndcapLayer2(key1, key2):
    if key1 == (2, 4) and (key2 == (3, 4) or key2 == (4, 4) or key2 == (5, 4)): return True
    return False


# f = open("scripts/moduleconnection_data.txt")
# f = open("scripts/moduleconnection_data_pt1_pt1p5.txt")
# f = open("scripts/moduleconnection_data_pt0p8_2p0.txt")
# f = open("scripts/moduleconnection_data_pt0p95_1p05.txt")
# f = open("scripts/moduleconnection_data_e0_200_pt0p8_2p0_pt0p95_1p05.txt")
# f = open("scripts/moduleconnection_data_e0_200_pt0p8_2p0_pt0p5_1p5.txt")
# f = open("scripts/moduleconnection_data_e0_200_pt0p8_2p0_pt0p5_1p5_400_pt0p8_2p0.txt")
# f = open("scripts/moduleconnection_data_e0_200_pt0p8_2p0_400_pt0p8_2p0.txt")
# f = open("scripts/moduleconnection_data_10_e0_200_100_pt0p8_2p0_400_pt0p8_2p0_nocurlers.txt")
f = open("scripts/moduleconnection_data_10_e0_200_100_pt0p8_2p0_400_pt0p8_2p0_nolossers.txt")
...
for index, line in enumerate(f.readlines()):
    ...
    # Adding connections for barrel to barrle and endcap to endcap
    for index in xrange(len(seq)-1):
        curr_layerkey = seq[index]
        next_layerkey = seq[index+1]

        if not (isOKConnBarrel(curr_layerkey, next_layerkey) or isOKConnEndcap(curr_layerkey, next_layerkey)):
            continue

        for ii in detid_db[curr_layerkey]:
            for jj in detid_db[next_layerkey]:
                orig = dict_from_file[ii[0]]
                targ = dict_from_file[jj[0]]
                origxy = math.sqrt(orig[0]**2 + orig[1]**2)
                targxy = math.sqrt(targ[0]**2 + targ[1]**2)
                # dr = math.sqrt((targ[0]-orig[0])**2 + (targ[1]-orig[1])**2)
                dr = targxy-origxy
                if ii not in all_connections:
                    all_connections[ii] = []
                    if dr < 35:
                        all_connections[ii].append(jj)
                    # if curr_layerkey == (5, 5):
                    #     if dr < 45:
                    #         all_connections[ii].append(jj)
                    #------------
                    # # To filter out connection with radian greater than 1 for (5,5) to (6,5) only
                    # # i.e. the returning tracks
                    # if (curr_layerkey == (5, 5) and next_layerkey == (6, 5)) or (curr_layerkey == (5, 5) and next_layerkey == (1, 4)):
                    #     orig = dict_from_file[ii[0]]
                    #     targ = dict_from_file[jj[0]]
                    #     # ax = [orig[0], orig[1]]
    ...
    # Adding connections for barrel to endcap
    for index in xrange(len(seq)-1):
        for jndex in xrange(index + 1, len(seq)):

            curr_layerkey = seq[index]
            next_layerkey = seq[jndex]

            if not (isOKConnBarrelEndcap(curr_layerkey, next_layerkey) or isOKConnEndcapEndcapLayer2(curr_layerkey, next_layerkey)):
                continue
...
for nodetype in nodetypes:
    ...
    h_nconn_dict[nodetype] = r.TH1F("nconn_{}_{}".format(nodetype[0], nodetype[1]), "", 200, 0, 200)
...
for i in all_connections:
    ...
    # print i, [ multiplicities[x] for x in multiplicities ]
    for x in multiplicities:
        coverages.append(multiplicities[x])

    # print i, multiplicities
    # continue

    # # Printing large conn modules
    # # Printing large conn modules
    # # Printing large conn modules
    # toprint = False
    # if len(multiplicities) > 20:
    #     toprint = True
    # if toprint:
    #     print i, multiplicities
    # if not toprint:
    ...
    # to filter out barrel 5,5 to 6,5 with radian greater than 1
